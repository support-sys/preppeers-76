...
async function createGoogleMeetWithCalendar(
  summary: string,
  description: string,
  startTime: string,
  endTime: string,
...
async function createManualMeetLink(
  accessToken: string,
  summary: string,
  startTime: string,
  endTime: string,
...
function createWorkingMeetLink(): string {
...
async function createEmergencyFallbackMeetLink(
  summary: string,
  description: string,
  startTime: string,
  endTime: string,
...
async function listAvailableCalendars(accessToken: string): Promise<any[]> {
  try {
...
async function handleTestRequest(req: Request): Promise<Response> {
  try {
    ...
    if (testType === 'calendar') {
      ...
      const result = await createGoogleMeetWithCalendar(
        testData.summary,
...
📄 Resume: ${interviewData.resume_url || 'Not provided'}
...
This is an automated calendar event created by Preppeers Interview System.
    ...
    const { meetLink, eventId } = await createGoogleMeetWithCalendar(
      `Mock Interview: ${interviewData.target_role}`,
      calendarDescription,
      startTime.toISOString(),
      endTime.toISOString(),
      [interviewData.candidate_email, interviewerEmail],
      interviewerName || 'Professional Interviewer',
      interviewData.candidate_name,
      interviewData.target_role,
      interviewData.plan_details
    );
    ...
    const { data: interview, error: interviewError } = await supabaseClient
      ...
      .insert({
        ...
        google_meet_link: meetLink, // Store the generated Meet link format
