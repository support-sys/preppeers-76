-- Keep current flow working, but use anon key + special header
-- This way we don't need service role key in database
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' AND OLD.iseligible = false AND NEW.iseligible = true THEN
    ...
    IF profile_record.full_name IS NOT NULL AND profile_record.email IS NOT NULL THEN
      ...
      -- Try to call the edge function with anon key + special header
      BEGIN
        PERFORM
          net.http_post(
