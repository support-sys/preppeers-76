...
const Interviewers = () => {
  ...
  const loadExistingData = async () => {
    ...
    const {
      data
    } = await supabase.from('interviewers').select('*').eq('user_id', user.id).single();
    if (data) {
      ...
      setInterviewerData({
        ...
        skillCategory: "",
        // This will be derived from skills
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const {
        error
      } = await supabase.from('interviewers').upsert({
        user_id: user.id,
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <Input id="experienceYears" name="experienceYears" type="number" value={interviewerData.experienceYears} onChange={handleInputChange} className="bg-white/10 border-white/20 text-white placeholder:text-slate-400" placeholder="Enter years of experience" required />
                </div>
                ...
                <div>
                  ...
                  <Input id="position" name="position" type="text" value={interviewerData.position} onChange={handleInputChange} className="bg-white/10 border-white/20 text-white placeholder:text-slate-400" placeholder="Enter your position" required />
                </div>
                ...
                <div>
                  ...
                  <Select value={interviewerData.skillCategory} onValueChange={handleSkillCategoryChange}>
                    ...
                    <SelectContent className="bg-slate-800 border-slate-600">
                      {Object.keys(skillOptions).map(category => <SelectItem key={category} value={category} className="text-white hover:bg-slate-700">
                          {category}
                        </SelectItem>)}
                    </SelectContent>
                  </Select>
                ...
                {/* Skills */}
                {interviewerData.skillCategory && <div>
                    <Label className="text-white">Skills *</Label>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      {skillOptions[interviewerData.skillCategory].map(skill => <div key={skill} className="flex items-center space-x-2">
                          <Checkbox id={skill} checked={interviewerData.skills.includes(skill)} onCheckedChange={() => handleSkillChange(skill)} className="bg-white/10 border-white/20 text-blue-500 focus:ring-0 focus:ring-offset-0" />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 md:grid-cols-4 gap-2">
                    {daysOfWeek.map(day => <div key={day} className="flex items-center space-x-2">
                        <Checkbox id={day} checked={interviewerData.availableDays.includes(day)} onCheckedChange={() => handleDayChange(day)} className="bg-white/10 border-white/20 text-blue-500 focus:ring-0 focus:ring-offset-0" />
                ...
                <div>
                  ...
                  <div className="flex space-x-4">
                    <div className="flex items-center space-x-2">
                      <Checkbox id="morning" checked={interviewerData.timeSlots.morning} onCheckedChange={() => handleTimeSlotChange("morning")} className="bg-white/10 border-white/20 text-blue-500 focus:ring-0 focus:ring-offset-0" />
                      <Label htmlFor="morning" className="text-white">Morning</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Checkbox id="afternoon" checked={interviewerData.timeSlots.afternoon} onCheckedChange={() => handleTimeSlotChange("afternoon")} className="bg-white/10 border-white/20 text-blue-500 focus:ring-0 focus:ring-offset-0" />
                      <Label htmlFor="afternoon" className="text-white">Afternoon</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Checkbox id="evening" checked={interviewerData.timeSlots.evening} onCheckedChange={() => handleTimeSlotChange("evening")} className="bg-white/10 border-white/20 text-blue-500 focus:ring-0 focus:ring-offset-0" />
                ...
                <div>
                  ...
                  <Textarea id="bio" name="bio" value={interviewerData.bio} onChange={handleInputChange} className="bg-white/10 border-white/20 text-white placeholder:text-slate-400 resize-none" placeholder="Write a short bio about yourself" rows={4} required />
                </div>

                {/* LinkedIn URL */}
                <div>
                  <Label htmlFor="linkedinUrl" className="text-white">LinkedIn URL</Label>
                  <Input id="linkedinUrl" name="linkedinUrl" type="url" value={interviewerData.linkedinUrl} onChange={handleInputChange} className="bg-white/10 border-white/20 text-white placeholder:text-slate-400" placeholder="Enter your LinkedIn URL" />
                </div>
